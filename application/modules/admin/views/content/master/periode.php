			...
			<div class="jarviswidget jarviswidget-color-darken" id="wid-id-2" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-custombutton="false">
				...
				<div>
					...
					<div class="widget-body">
						...
						<form action="<?php echo base_url(); ?>admin/create/master/periode" method="post">
								...
								<div class="row">
									<section class="col-sm-6">
										<div class="form-group">
											<label>Panitia Program Study</label>
											<select style="width:100%" class="select2" id="jur">
									...
									<section class="col-sm-6">
										<div class="form-group">
											...
											<select style="width:100%" class="select2" name="idper">
												<option value="0" selected="" disabled=""></option>
												<option value="<?php echo date('Y').'1' ?>"><?php echo 'Tahun '.date('Y').' Semester Ganjil (1)' ?></option>
												<option value="<?php echo date('Y').'2' ?>"><?php echo 'Tahun '.date('Y').' Semester Genap (2)' ?></option>
											</select>
										</div>
									</section>
									...
									<section class="col-sm-12">
										<div class="form-group">
											...
											<select multiple style="width:100%" class="select2" id="panitia" name="idpan[]">
											</select>
										</div>
									</section>
								</div>
				...
				<div>
					...
					<div class="widget-body no-padding">
					    ...
					    <thead>
					      ...
					      <tr>
				          <th data-class="expand">ID Periode</th>
				          <th data-class="expand">Nama Panitia</th>
									<th data-class="expand">Jabatan</th>
									<th data-class="expand">Action</th>
					      </tr>
					    </thead>
					      <tbody>
									<?php foreach ($get_periode as $v_per): ?>
					          <tr>
					            <td><?php echo $v_per['id_periode'] ?></td>
					            <td><?php echo $v_per['nama'] ?></td>
											<td><?php echo $v_per['tipe_panitia'] ?></td>
											<td>
													<button type="button" onclick="del('<?php echo $v_per['id_per'] ?>')" name="jurdel" class="btn btn-danger"><i class="fa  fa-trash-o"></i></button>												
											</td>
					          </tr>
									<?php endforeach; ?>
					      </tbody>
					...
					<!-- Modal Edit -->
					<div class="modal fade" id="peredit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
						<div class="modal-dialog" role="document">
							<div class="modal-content">
...
<script>
	function del(ID) {
	   if (confirm("Apakah Anda ingin menghapus data ini ?") == true) {
	       window.location.href='<?php echo base_url() . "admin/delete/master/periode/"?>'+ID;
	   } else {
	       return FALSE;
	   }
	}
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#jur').change(function() {
		 ...
		 $.ajax({
		 	type: 'GET',
			url: '<?php echo base_url(); ?>api/panitia?id=' + jur,
			success: function(r) {
					var html='';
					for (var i = 0; i < r.length; i++) {
						html += '<option value="'+r[i].id_panitia+'">'+r[i].nama+'</option>';
					}
					$('#panitia').html(html);
				}
			});
...
<script type="text/javascript">
	...
	var pagefunction = function() {
		...
		*/
		...
		loadScript("<?php echo base_url(); ?>assets/smartadmin/js/plugin/bootstrap-duallistbox/jquery.bootstrap-duallistbox.min.js", initializeDuallistbox);
