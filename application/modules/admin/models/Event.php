<?php
  ...
  class Event extends CI_Model
  {
    ...
    public function get() {
      // print_r($this->session->userdata('jur')); exit;
      $this->session->userdata('jur') !='' ? $query = $this->db->get_where('v_event' , array('id_jurusan' => $this->session->userdata('jur'))) :   $query = $this->db->get('v_event');
    ...
    public function update($id) {
      ...
      $s = '';
      foreach ($r2->result_array() as $a) {
        $s = $a['id_event'];
      }

      if ($r2->num_rows() == 1 && $s != $id) {
        return $mssg = "<SCRIPT LANGUAGE='JavaScript'>
              window.alert('Lokasi yang Anda masukan sudah digunakan pada tanggal tersebut')
              window.location.href='".base_url()."Admin#A_content/master/event';
              </SCRIPT>";
